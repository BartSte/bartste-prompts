...
from prompts import _instructions
...
def _add_options(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        "--filetype",
        default="",
        choices=_get_file_names(path.module(_instructions.edit_instructions)),
        help=(
...
def _func(args: argparse.Namespace):
    ...
    action_factory: ActionFactory = ActionFactory(args.action)
    prompt_factory: PromptFactory = PromptFactory(**kwargs)

    prompt: "Prompt" = prompt_factory.create()
    action: "AbstractAction" = action_factory.create(prompt, **kwargs)
    action()
